...
async function run() {
  ...
  if (!fs.existsSync(jobsPath)) {
      console.error("jobs_batch_19.json not found!");
      return;
  }
  
  const jobsData = JSON.parse(fs.readFileSync(jobsPath, 'utf8'));

  console.log(`🚀 Processing ${jobsData.length} tailored jobs...`);
  ...
  for (let i = 0; i < jobsData.length; i++) {
    ...
    if (job.email) {
      ...
      const fullBody = `<p>Hi ${job.company} Hiring Team,</p>${job.bodyHtml}<p>${p3}</p><p>Best,</p>`;
      ...
      const payload = { 
          company: job.company, 
          role: job.role, 
          channel: "Batch Processor 19", 
          telegramId: `batch19-${job.id}`, 
          email: job.email, 
          status: 'applied', 
          type: 'web', 
          appliedDate: new Date().toISOString(), 
          description: `<b>SUBJECT: ${job.subject}</b><br><br>${fullBody}` 
      };
      
      await syncToDashboard(payload);
    } else if (job.link || job.notes) {
      console.log(`[${i+1}/${jobsData.length}] Adding manual entry for ${job.company}...`);
      const payload: any = { 
          company: job.company, 
          role: job.role, 
          channel: "Batch Processor 19", 
          telegramId: `batch19-${job.id}`, 
          description: "Manual Application Extraction - Batch 19", 
          status: 'to_apply', 
          type: 'web', 
          appliedDate: new Date().toISOString(), 
          email: '' 
