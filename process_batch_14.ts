...
async function run() {
  const jobsPath = path.join(process.cwd(), 'jobs_batch_14.json');
  if (!fs.existsSync(jobsPath)) {
      console.error("jobs_batch_14.json not found!");
      return;
  }
  ...
  for (let i = 0; i < jobsData.length; i++) {
    ...
    if (job.email) {
      ...
      const payload = { 
          company: job.company, 
          role: job.role, 
          channel: "Batch Processor 14", 
          telegramId: `batch14-${job.id}`, 
          email: job.email, 
          status: 'applied', 
          type: 'web', 
      ...
      const payload = { 
          ...
          telegramId: `batch14-${job.id}`, 
          link: job.link, 
          description: "Manual Link Extraction - Batch 14", 
          status: 'to_apply', 
          type: 'web', 
          appliedDate: new Date().toISOString(), 
          email: '' 
